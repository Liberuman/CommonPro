...
public class UrlConfigManager {
	...
	private static void fetchUrlDataFromXml(final Activity activity,final int resourceId) {
	...
	public static URLData findURL(final Activity activity, 
			final String findKey,final int resourceId) {
		// 如果urlList还没有数据（第一次），或者被回收了，那么（重新）加载xml
		if (urlList == null || urlList.isEmpty())
			fetchUrlDataFromXml(activity,resourceId);
